 ...
 */
class Flag extends FoodTalkActiveRecord
{
    /**
    ...
    /**
     * Returns query to get all flags of a post and related users
     */
    public static function getQuery($imagePath, $thumbPath, $postId, $userId)
    {
        ...
        $sql .= " FROM user u JOIN `flag` f ON u.id = f.userId";
        $sql .= " LEFT JOIN session s ON s.userId = u.id";
        $sql .= " WHERE f.postId = $postId";
